...
/// Convert exact Rappen to CHF as a Double — **presentation only**, used solely
/// for plotting. All real money math stays in Int64 Rappen.
...
/// A polished donut: a clean, evenly inset ring with a centred headline value, and
/// a compact legend that lists each slice with its amount and share of the total.
/// The legend is kept to a sensible width next to the chart (rather than stretched
/// to the card's edge) so labels and amounts read together.
struct AllocationDonut: View {
    ...
    var body: some View {
        HStack(alignment: .center, spacing: KontivaTheme.Space.xl) {
            donut
            legend.frame(maxWidth: 380)
            Spacer(minLength: 0)
        }
    }
        ...
        .overlay {
            VStack(spacing: 2) {
                ...
                if let centerCaption {
                    Text(centerCaption)
                        .font(.system(size: 9.5)).foregroundStyle(KontivaTheme.textTertiary)
                        .lineLimit(1)
                }
            }
            .frame(width: 132)
...
/// Donut showing where this month's net income goes: fixed costs, variable
/// budgets, bills, savings, and what's left available. Center shows the available
/// amount.
...
/// A line+area trend of a money value over months (e.g. available balance).
...
/// A tiny, axis-less sparkline for an at-a-glance trend (e.g. next to a headline).
