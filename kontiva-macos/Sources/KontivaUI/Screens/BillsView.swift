...
/// (overdue → due this month → upcoming → paid). Card-based to match Planung and
/// Sparen, with an at-a-glance summary and a tap-to-tick "mark paid" checkbox.
struct BillsView: View {
    ...
    private func billRow(_ bill: OneOffBill, state: BillState, accent: Color) -> some View {
        ...
        return HStack(spacing: KontivaTheme.Space.sm) {
            ...
            .buttonStyle(.plain)
            .help(loc(isPaid ? .billsMarkOpen : .billsMarkPaid))
