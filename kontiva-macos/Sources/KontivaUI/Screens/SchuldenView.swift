...
/// debts. The goal is to *help clear* debt — hence the guidance card with
/// legally-grounded, plain-language steps. It is information, not legal advice.
struct SchuldenView: View {
    ...
    var body: some View {
        ...
        .navigationSubtitle(loc(.schuldenSubtitle))
        .toolbar {
            ToolbarItemGroup(placement: .primaryAction) {
                Button { editing = .edit(nil) } label: {
                    Label(loc(.commonAdd), systemImage: "plus")
                }
                .symbolEffect(.bounce, value: model.debts.count)
            }
        }
        .onChange(of: model.newItemSignal) { editing = .edit(nil) }
    ...
    // MARK: Overdue bills (auto from Rechnungen)
        ...
        return KontivaCard {
            VStack(alignment: .leading, spacing: KontivaTheme.Space.md) {
                HStack(spacing: KontivaTheme.Space.sm) {
                    ...
                    Text(loc(.schuldenOverdueBills))
                        .font(.title3.weight(.semibold)).foregroundStyle(KontivaTheme.textPrimary)
                ...
                VStack(spacing: 0) {
                    ForEach(Array(items.enumerated()), id: \.element.id) { idx, bill in
                        ...
                        row(icon: "doc.text.fill", color: KontivaTheme.swissRed,
                            ...
                            .onTapGesture { model.selection = .bills }   // managed in Rechnungen
                    }
                }
                Text(loc(.schuldenManagedInBills))
                    .font(.caption).foregroundStyle(KontivaTheme.textTertiary)
            }
        }
    }

    // MARK: Recorded debts, grouped by type
        ...
        return KontivaCard {
            VStack(alignment: .leading, spacing: KontivaTheme.Space.md) {
                HStack(spacing: KontivaTheme.Space.sm) {
                    ...
                    Text(type.localizedName(loc.localization))
                        .font(.title3.weight(.semibold)).foregroundStyle(KontivaTheme.textPrimary)
        ...
        KontivaCard {
            VStack(alignment: .leading, spacing: KontivaTheme.Space.md) {
                HStack(spacing: KontivaTheme.Space.sm) {
                    ...
                    Text(loc(.schuldenGuidanceTitle))
                        .font(.title3.weight(.semibold)).foregroundStyle(KontivaTheme.textPrimary)
                    Spacer(minLength: 0)
                }
                Divider()
